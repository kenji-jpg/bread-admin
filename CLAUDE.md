...
- **【完成】已寄出通知（單筆）+ 一步完成**：結帳單按「已寄出」的行為依出貨方式分流（前端 `handleMarkShipped`，`app/admin/t/[slug]/checkouts/page.tsx`）：
  - **宅配/店到店**：此時已收款（mark_ordered 已設 paid），寄出即終態 → **直接 `markCompleted`（ordered→completed）** 並推 `notify-checkout-v1` 的 `kind='shipped'` LINE 通知（宅配/店到店文案，**不含物流/寄件編號**，純告知已出貨）。省掉再按一次「已完成」。
  - **賣貨便**：取貨付款、走 7-11 email 完成 → 已寄出只 `markShipped`（→shipped）、**不通知不完成**（由 7-11 自行發取貨通知；edge fn v19 對 myship 也回 `skipped`）。
  - 通知成功寫 `checkouts.shipped_notified_at` 戳記；套 `message_config` 結尾署名（只 append footer，不跑「3 天」replace）。客人未綁 LINE / 發送失敗 → 靜默降級（訂單仍完成，toast 提示）。`useCheckout.notifyShipped`。

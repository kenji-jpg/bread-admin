...
└── check-subscription-expiry/  # 定時任務：訂閱到期自動降級（每日凌晨 2 點）
workers/
...
- `check-subscription-expiry` — 定時任務：檢查 Pro 方案到期，自動降級為 basic（每日凌晨 2 點）
...
- `use-liff.tsx` 的分享連結函數支援 `customLiffId` 參數
- **待完成**：後台設定頁加輸入框、LIFF 初始化從租戶動態取得 liff_id
...
- 到期自動降級：`check-subscription-expiry` Edge Function 每日執行
...
| 3 | **自訂 404 / Error 頁面** | `not-found.tsx` + `error.tsx`，目前打錯網址顯示 Next.js 預設 | 2 小時 |
| 4 | **CSV 匯出** | 訂單/結帳單/商品列表下載按鈕目前無功能（UI 已有 icon） | 1 天 |
| 5 | **訂閱到期提醒 Email** | Edge Function 已自動降級但不會提前通知店家 | 1 天 |
...
| 9 | ~~多租戶 LIFF 隔離~~ | ✅ 已完成 — `LiffProvider` 動態取租戶 `liff_id` | — |
...
- **【完成】方案系統（Basic/Pro/Max）**：`tenants.plan` 欄位（三層）、前端 permission hook、sidebar 鎖定、超管升降級 RPC、Max 方案 Realtime 獨佔、到期自動降級 Edge Function
- **【完成】超管付款管理**：`/admin/payments` 列表 + 驗證頁面，升級方案操作

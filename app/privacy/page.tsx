...
const DEFAULT_SECTIONS: ContentSection[] = [
    { title: '1. 前言', body: 'PlusHub 重視您的隱私。', items: null },
]

export default async function PrivacyPage() {
    const supabase = await createClient()
    const { data } = await supabase.rpc('get_platform_content_v1', { p_content_type: 'privacy' })

    const title = data?.success ? data.title : '隱私政策'

-- 修正 A1：達免運時 shipping_fee 設為 0（不要保留 $80），讓 total = items_sum_incl_refund
--   - 達免運（有退款品項）→ shipping_fee = 0，total = items_sum_with_refund（= 商品 sum 扣免運抵扣）
--   - 未達免運（無退款品項）→ shipping_fee = base (80/60)，total = items_sum + shipping_fee
--
-- 原因：之前保留 shipping_fee = 80 並加退款 -80，兩者抵消讓 total 卡在「商品 sum」，
-- 客人 net cash 變成「商品 sum」而非「商品 sum 扣免運抵扣」，等於免運實際沒兌現。
-- 此修正讓 total 真實反映客人應付金額，補匯金額也會自動正確。
...
BEGIN
    ...
    -- ★ 達免運（有退款品項）→ shipping_fee = 0；否則 = 基準值
    IF v_refund_item_id IS NOT NULL THEN

...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(row_data ORDER BY
      CASE WHEN p_sort='amount_desc' THEN s_total END DESC NULLS LAST,
      CASE WHEN p_sort='amount_asc'  THEN s_total END ASC  NULLS LAST,
      CASE WHEN p_sort='owed_desc'   THEN s_owed  END DESC NULLS LAST,
      s_created DESC
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'paid_amount', COALESCE(c.paid_amount, 0),
      'outstanding_amount', GREATEST(0, c.total_amount - COALESCE(c.paid_amount, 0)),
    ...
    c.created_at AS s_created,
    c.total_amount AS s_total,
    GREATEST(0, c.total_amount - COALESCE(c.paid_amount, 0)) AS s_owed
    FROM checkouts c LEFT JOIN members m ON c.member_id = m.id
    WHERE c.tenant_id = p_tenant_id
      AND (p_shipping_status IS NULL OR c.shipping_status = p_shipping_status)
      AND (p_payment_status IS NULL OR c.payment_status = p_payment_status)
    ...
    ORDER BY
      CASE WHEN p_sort='amount_desc' THEN c.total_amount END DESC NULLS LAST,
      CASE WHEN p_sort='amount_asc'  THEN c.total_amount END ASC  NULLS LAST,
      CASE WHEN p_sort='owed_desc'   THEN GREATEST(0, c.total_amount - COALESCE(c.paid_amount, 0)) END DESC NULLS LAST,
      c.created_at DESC

-- list_checkouts_v1 回傳多加 outstanding_amount = GREATEST(0, total_amount - paid_amount)
-- 用途：賣貨便插件開賣場時用「尚欠額」當賣場價格（預付後只收尾款，避免客人付兩次運費）。
-- 純附加欄位，後台原有用途不受影響；沒預付的單 outstanding_amount = total_amount。

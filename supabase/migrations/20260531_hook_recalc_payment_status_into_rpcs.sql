-- 在 merge/link/remove 三支入口 RPC 加 PERFORM recalc_checkout_payment_status_v1
-- 順序：原本邏輯 → PERFORM apply_free_shipping_refund_v1 → PERFORM recalc → 重抓 total 回傳
-- 另外：
--   - merge_checkouts_v1：合併時 paid_amount 累加（primary 拿到 SUM(secondaries)），交給 recalc 推導狀態
--   - link_order_items_to_checkout_v1：加品項自動 is_notified=false 重置
--   - remove_checkout_item_v1：移除後 recalc
...
BEGIN
  ...
  -- paid_amount 合併：primary 拿到所有 secondaries 的 paid_amount 總和
  UPDATE checkouts c
    SET paid_amount = (
      SELECT COALESCE(SUM(c2.paid_amount), 0)
      FROM checkouts c2
      WHERE c2.id = ANY(p_checkout_ids) AND c2.tenant_id = p_tenant_id
    )
  WHERE c.id = v_primary_id AND c.tenant_id = p_tenant_id;
  ...
  -- payment_status 不在這裡硬設，交給後面 recalc 推導

  PERFORM apply_free_shipping_refund_v1(p_tenant_id, v_primary_id);
  PERFORM recalc_checkout_payment_status_v1(p_tenant_id, v_primary_id);
...
BEGIN
    ...
    UPDATE checkouts c SET shipping_method = v_new_shipping_method, shipping_fee = v_new_shipping_fee,
        ...
        is_notified = false,  -- 加品項後一律重置通知狀態

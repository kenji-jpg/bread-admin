-- 對稱重算結帳單的免運退款品項：
-- - 商品小計 >= 免運門檻 且 沒有退款品項 → INSERT 退款品項（負金額）
-- - 商品小計 < 免運門檻 且 有退款品項 → DELETE 退款品項
-- - 同時確保 shipping_fee 維持基準值（80/60），不再因為達標而歸零
-- 只處理 delivery / seven_store；myship/myship_free 由 merge 邏輯處理；pickup 無運費
...
BEGIN
    -- 1. 取結帳單
    SELECT id, member_id, shipping_method, shipping_status, shipping_fee, payment_status
    INTO v_checkout
    FROM checkouts
    ...
    -- 3. 只處理 delivery / seven_store；其他略過
    ...
    -- 6. 既有退款品項？（用 item_name + product_id IS NULL + unit_price < 0 比對）
    ...
    -- 8. 對稱重算
    IF v_goods_subtotal >= v_threshold AND v_refund_item_id IS NULL THEN
        ...
        v_action := 'inserted';
    ELSIF v_goods_subtotal < v_threshold AND v_refund_item_id IS NOT NULL THEN
        DELETE FROM order_items WHERE id = v_refund_item_id;
        v_refund_item_id := NULL;
        v_action := 'deleted';
    END IF;

    -- 9. 確保 shipping_fee 是基準值（修正可能被舊 merge 邏輯歸零的情況）
    IF v_checkout.shipping_fee IS DISTINCT FROM v_base_shipping_fee THEN
        UPDATE checkouts
        SET shipping_fee = v_base_shipping_fee, updated_at = NOW()
        WHERE id = p_checkout_id;
    END IF;

    -- 10. 重算 total_amount / item_count / checkout_items JSON
    ...
    v_new_total := v_new_total + v_base_shipping_fee;
    ...
    RETURN jsonb_build_object(
        ...
        'shipping_fee', v_base_shipping_fee,
...
GRANT EXECUTE ON FUNCTION public.apply_free_shipping_refund_v1(uuid, uuid) TO authenticated, service_role;

// ============================================
// 🔔 Edge Function: notify-myship-url (v12)
// v12: 賣場開立通知支援 — (1) 賣貨便免運顯示「✨賣貨便免運 -$38」+「免運取貨付款」
//      (2) 金額改用尚欠額(總額−已付)，預付過顯示「已收 $X，本次付尾款」
// v11: 接收 myship_account_name 並寫入欄位
// ============================================
...
serve(async (req) => {
  ...
  try {
    ...
    if (!token) {
      return json({ success: false, error: 'unauthorized', message: '缺少 Authorization header' }, 401)
    }
    ...
    if (!tenant_id || !checkout_id || !store_url) {
      return json({
        ...
        message: '缺少必要參數：tenant_id, checkout_id, store_url',
      }, 400)
    }
    ...
    const { data: rpcResult, error: rpcError } = await supabase.rpc(
      'set_myship_url_v1',
      {
        ...
        p_store_url: store_url,
        p_myship_store_name: myship_store_name || null,
        p_myship_account_name: myship_account_name || null,
      }
    )
    ...
    // 商品明細：優先順序 products.name → auction_orders.product_name → item_name → '商品'
    let itemsText = ''
    try {
      ...
      if (checkoutItems && checkoutItems.length > 0) {
        ...
        for (const item of checkoutItems as any[]) {
          const productName =
            item.products?.name ||
            item.auction_order?.[0]?.product_name ||
            item.item_name ||
            '商品'
    ...
    // 取出貨方式 + 已付，決定免運顯示與尚欠額（賣場收的是尚欠額，不是全額）
    const { data: ck } = await supabase
      .from('checkouts')
      .select('shipping_method, total_amount, paid_amount')
    ...
    const { error: notifyRpcErr } = await supabase.rpc(
      'update_checkout_notify_status_v1',
      {
        p_tenant_id: tenant_id,
        p_checkout_id: checkout_id,
        p_notify_status: notifyStatus,
        p_notify_error: pushResult.error || null,
      }
    )

    if (notifyRpcErr) {
      console.error('[Notify] notify status write-back failed:', notifyRpcErr)
    }

    console.log(
      `[Notify] ✅ checkout=${checkout_no}, notify=${notifyStatus}, customer=${customer_name}, items=${itemsText ? 'yes' : 'no'}, account=${myship_account_name || '-'}, user=${user.id}`
    )
    ...
    return json({
      ...
      customer_name,
      store_url,
      myship_store_name: myship_store_name || null,
      myship_account_name: myship_account_name || null,
      notify_status: notifyStatus,
      notify_error: pushResult.ok ? null : pushResult.error,
    })
  } catch (error) {
    console.error('[Notify] Unhandled error:', error)
    return json(
      { success: false, error: 'internal_error', message: (error as Error).message },
      500
    )
  }
})
